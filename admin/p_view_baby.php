            ...
            <div class="col-md-10">
                <table class="table">
                    ...
                    $actions = array(
                        'edit' => array(
                            'label' => 'Edit',
                            'link' => 'p_edit_baby.php?pid='.$id.'&',
                            'params' => array('id' => 'b_id'),
                            'attributes' => array('class' => 'btn btn-success')
                        ),
                        'delete' => array(
                            'label' => 'Delete',
                            'link' => 'p_delete_baby.php?pid='.$id.'&',
                            'params' => array('id' => 'b_id'),
                            'attributes' => array('class' => 'btn btn-success')
                        )
                    );
                    ...
                    $join=array( 
                        ...
                        $con="pid='$id' AND status=1";
                    // Fields for the query
                    $fields = array('b_id','name','parent_names', 'date_of_birth','gender',  'blood_group', 'height',
                     'weight', 'medical_conditions');

                    $vaccines = $dao->selectAsTable($fields, 'baby_details',$con, $join, $actions, $config);
                    
                    // Display the table with data
                    if(!empty( $vaccines))
                      echo $vaccines;
                   else
                      echo"</table><h2 style='text-align:center'>No Baby Details Are Added</h2>";
                    ?>
                </table>
            </div>    
        </div><!-- End row -->
